...
export const sendToSingleDevice = async (bodyData: any) => {
...
export const sendToSingleDeviceViaId = async (userId: string) => {


    const titleBody = " ultrasonic sensor"
    const msgBody = "coming value: 55cm"
    const imgBody = "dsjfdlsk"
    const userData = fcmConfig.fcmDevices.filter((userData) => userData.id == userId)
    ...
    if (fcmConfig.fcmDevices.length > 0) {
        ...
        return { message: "in server there should be atleast 1 device's fcm data" }
    }
}
